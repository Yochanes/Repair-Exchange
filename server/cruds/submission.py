...
from models.submission import Order, ServiceRequest, Offer, SubmissionFeedback, StatusEnum
from models.user import Master
from models.relationship import OrderRepair
from services.main import AppCRUD
from utils.app_exceptions import AppException
from schemas.submission import OrderIn, OrderEdit, RequestIn, RequestEdit, OfferIn, OfferEdit, FeedbackIn, FeedbackEdit


class SubmissionCRUD(AppCRUD):
    async def create_order(self, data: OrderIn, user: models.user.Client, bg_tasks: BackgroundTasks) -> Order:
        order = Order(client_id=user.id, client_message=data.client_message, client_price=data.client_price,
                      master_username=data.master_username)
        self.db.add(order)
        self.db.commit()
        self.db.refresh(order)
        for repair in data.repairs_id:
            order_repair = self.db.query(OrderRepair).filter(OrderRepair.repair_id == repair,
                                                             OrderRepair.order_id == order.id).first()
            if order_repair:
                continue
        ...
        bg_tasks.add_task(UserCRUD(self.db).send_mailing, data.master_username, order_id=order.id, request_id=None)
        return order
    ...
    def get_orders_by_client_id(self, client_id: int) -> List[Order]:
        orders = self.db.query(Order).filter(Order.client_id == client_id).order_by(Order.created_at).all()
    ...
    def finish_order_by_id(self, id: int, status: StatusEnum, user: models.user.Client) -> dict | Exception:
        ...
        if status == StatusEnum.canceled:
            ...
            UserCRUD(self.db).charge_commission(master.get('username'), order.client_price)
        elif status == StatusEnum.submitted:
            if order.status != StatusEnum.completed:
                return AppException.ValidationException('Дождитесь подтверждения выполнения работы мастером!')
            order.status = status
    ...
    async def create_request(self, data: RequestIn, user: models.user.Client, bg_tasks: BackgroundTasks,
        ...
        request = ServiceRequest(client_id=user.id, title=data.title, description=data.description,
                                 client_price=data.client_price, service_type_id=data.service_type_id,
        ...
        if files:
            for file in files:
                if os.path.exists(f'media/files/{file.filename}'):
                    i = 1
                    paths_parts = [file.filename[:file.filename.rindex('.')],
                                   file.filename[file.filename.rindex('.') + 1:]]
                    while os.path.exists(f'media/files/{paths_parts[0]}({i}).{paths_parts[1]}'):
                        i += 1
                    with open(f'media/files/{paths_parts[0]}({i}).{paths_parts[1]}', 'wb') as f:
        ...
        bg_tasks.add_task(UserCRUD(self.db).send_mailing, '__all__', order_id=None, request_id=request.id)
        return request
    ...
    def get_requests(self, user_id: int) -> List[ServiceRequest]:
        requests = self.db.query(ServiceRequest).filter(ServiceRequest.status != 'В процессе',
                                                        ServiceRequest.client_id != user_id)\
            .order_by(ServiceRequest.created_at).all()
        return list(requests)[::-1]

    def get_requests_by_client_id(self, client_id) -> List[ServiceRequest]:
        requests = self.db.query(ServiceRequest).filter(ServiceRequest.client_id == client_id).\
            order_by(ServiceRequest.created_at).all()
    ...
    def update_request_by_id(self, id: int, data: RequestEdit, user_id: int) \
        ...
        if request.status == StatusEnum.processing:
            ...
            if data.status == StatusEnum.submitted:
                return AppException.ValidationException('Дождитесь, пока мастер подтвердит выполнение заказа!')
            request.status = StatusEnum.canceled
            offer = self.db.query(Offer).filter(Offer.request_id == request.id, Offer.is_accepted is True).first()
            if not offer:
    ...
    def complete_request_by_id(self, id: int, master: Master, status: StatusEnum):
        ...
        offer = self.db.query(Offer).filter(Offer.request_id == request.id, Offer.master_username == master.username,
    ...
    def create_offer(self, data: OfferIn, master_username: str) -> Offer | Exception:
        ...
        offer = Offer(master_username=master_username, message=data.message,
                      request_id=data.request_id, price=data.price, time=data.time)
        self.db.add(offer)
        request = self.db.query(ServiceRequest).filter(ServiceRequest.id == data.request_id).first()
        if not request:
            return AppException.NotFoundException('Заказ не найден!')
    ...
    def accept_offer(self, id: int, client: models.user.Client) -> Exception | dict:
        ...
        if offer.request.client_id != client.id:
            return AppException.ForbiddenException('Нет доступа!')
        if offer.master.stats != "available":
            return AppException.BadRequestException('Данный мастер недоступен')
    ...
    def create_feedback(self, data: FeedbackIn, user_id: int, pictures: List[UploadFile] = None) -> SubmissionFeedback | Exception:
        ...
        new_feedback = SubmissionFeedback(client_id=user_id,
                                          rating=data.rating,
        ...
        master = self.db.query(Master).filter(Master.username == new_feedback.master_username).first()
        if not master:
    ...
    def get_feedback_by_offer(self, offer_id: int | None, order_id: int | None, user_id: int = None,
        ...
        if not (feedback.client_id == user_id or feedback.master_username == master_username):
            return AppException.ForbiddenException('Нет доступа!')
        return feedback

    def get_feedbacks_by_master(self, master_username: str) -> List[SubmissionFeedback]:
        feedbacks = self.db.query(SubmissionFeedback).filter(SubmissionFeedback.master_username == master_username) \
            .all()
    ...
    def delete_feedback_by_id(self, id: int, user_id: int) -> str | Exception:
        ...
        master = self.db.query(Master).filter(Master.username == feedback.master_username).first()

        sum_of_ratings = master.rating * master.number_of_feedbacks
        sum_of_ratings -= feedback.rating
        master.number_of_feedbacks -= 1
        if master.number_of_feedbacks != 0:
            master.rating = sum_of_ratings / master.number_of_feedbacks
        else:
            master.rating = 0
    ...
    def get_requests_by_master_username(self, master: Master) -> List[ServiceRequest]:
        offers = self.db.query(Offer).filter(Offer.master_username == master.username).all()
        new_requests = list()
        for offer in offers:
