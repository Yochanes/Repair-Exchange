...
import { getFeedback, createFeedback, deleteFeedback, updateFeedback } from "../../services/feedback.service";
        ...
        deleteFeedback(feedbackId)
            .then((response) => {
                if (response.status === 200) {
                    ...
                    console.error('Ошибка при удалении отзыва:', response.message)
                }
            })
            .catch(error => {
                // В случае ошибки запроса выводим сообщение об ошибке
                console.log('Ошибка при удалении отзыва:', error.message)
            })
    }

    // Редактирование отзыва
    const onUpdateFeedback = (feedbackId, updatedData) => {
        updateFeedback(feedbackId, updatedData)
            .then(() => {
                getFeedback(username).then(setFeedback)
            })
            .catch(error => {
                console.error('Ошибка при редактировании отзыв:', error)
            })
    }
                        ...
                        <div className="info_prof-kiril font_inter">
                            {feedback.map((v) => (
                                <React.Fragment key={v.id}>
                                    ...
                                    <div className="client-comment__actions">
                                        <button onClick={() => onDeleteFeedback(v.id)}>Удалить</button>
                                        <Popup
                                            ...
                                            {close => (
                                                <div className="modal-content" style={{ width: "600px" }}>
                                                    ...
                                                    <div className="containerPopUp">
                                                        ...
                                                        <div className="popUpBtnffeee">
                                                            <button className="model-rewbtn_6PopUp inter" onClick={(e) => { onUpdateFeedback(v.id, { data: { master_username: username, rating, description: fbtext }, files: fileInputRef.current.files }); close(e) }}>Сохранить</button>
                                                            <button className="model-rewbtn_6PopUpBack inter " onClick={close}>Отмена</button>
                                                        </div>
                                                    </div>
                                                </div> 
                                            )}
